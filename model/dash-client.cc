...
DashClient::MessageReceived (Packet message)
{
  ...
  if (Simulator::Now().GetSeconds() > m_time){
    double temp;
    if ((m_countMessage % 100) != 0){
      temp = m_segment_bytes/(m_countMessage % 100);
    } else {
      temp = m_segment_bytes/100;
    }
    ...
    std::cout << "LOG1,"
              ...
              << "," << m_time 
              << "," << m_id          //Nodeid
              << "," << (m_bitRate + old05) / 2                             //newBitRate
              << "," << old05                                          //oldBitRate
              << "," << "x"
              << "," << m_player.m_interruption_time.GetSeconds ()                  //interTime
              << "," << (m_totBytes * 8)/(m_countMessage/50.0)
              << std::endl;  
    m_time += 0.5;
    old05 = m_bitRate;
  }
  ...
  if (mpegHeader.GetFrameId () == MPEG_FRAMES_PER_SEGMENT - 1)
    {
      ...
      std::cout << "LOG2,"
                << m_segment_bytes/100       //bytes
                //<< m_segmentFetchTime.GetSeconds () << "\t"                  //segmentTime
                //<< "\t" << 8 * m_segment_bytes / m_segmentFetchTime.GetSeconds () << "\t"     //segmentRate
                //<< "\t" << m_totBytes << "\t"                              //total bytes received
                //<< "\t" << message.GetSize() << "\t"                           //packet dimension
                << "," << m_countMessage << ",";
      ...
      m_throughput += m_bitRate;

      std::cout << Simulator::Now ().GetSeconds ()        //time
                << "," << m_id          //Nodeid
                << "," << m_bitRate                                //newBitRate
                << "," << old                                          //oldBitRate
                << "," << GetBitRateEstimate ()                        //estBitRate
                << "," << m_player.m_interruption_time.GetSeconds ()                  //interTime
                //<< "\t" << currDt.GetSeconds ()                                      //T
                //<< "\t" << (m_lastDt >= Time ("0s") ? (currDt - m_lastDt).GetSeconds () : 0)     //dT
                //<< "\t" << bufferDelay.GetSeconds ()                       //del
                << "," << m_throughput/(m_countMessage/100.0)
                << std::endl;
